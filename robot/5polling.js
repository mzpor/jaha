...
📚 **قابلیت‌های اصلی:**
• مدیریت انجام گزارش روزانه
• ارزیابی و نظر سنجی
• مدیریت دبیران
• گزارش روزانه و ماهانه
• گزارش‌گیری از پژوهشها
...
⏰ ${getTimeStamp()}`;
    ...
    if (!isAdmin(msg.from.id) && !isGroupAdmin(msg.from.id)) {
      ...
      if (groups.length === 0) {
        reply = '📝 هیچ گروهی یافت نشد.\n\nبرای شروع، لطفاً اعضا در گروه‌ها /عضو شوند.';
...
🤖 این ربات برای فعالیت‌های جهادی طراحی شده است
📚 قابلیت‌های اصلی:
• مدیریت حضور و غیاب
• ارزیابی و نظر سنجی
• مدیریت گروه‌ها
...
⏰ ${getTimeStamp()}`;
            ...
            if (!isAdmin(callback_query.from.id)) {
              await answerCallbackQuery(callback_query.id, '⚠️ فقط مدیر مرکز می‌تواند این کار را انجام دهد.', true);
              return;
            }
...
⏰ ${getTimeStamp()}`;
            ...
            const reply = `🏫 *معرفی مرکز جهادی*

🌟 مرکزی معتبر با ۱۰+ سال سابقه در فعالیت‌های جهادی
...
⏰ ${getTimeStamp()}`;
            ...
            if (!isAdmin(callback_query.from.id)) {
              await answerCallbackQuery(callback_query.id, '⚠️ فقط مدیر مرکز می‌تواند نقش‌ها را مدیریت کند.');
            } else {
